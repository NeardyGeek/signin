...
.then((res) =>{
                            var stocklist = document.getElementById("stockList");
                            stocklist.innerHTML = "<h1>Portfolio (" + res[1] + ")</h1><ul>"+res[0]+"</ul>";
                        })
     ...
     if(t >= 9.5 && t <= 20){
            ...
            useEffect(() => {
                const interval =  setInterval(() => {
                    setSeconds(db.collection("users").doc(uid).collection("tickers").get()
                    .then(querySnapshot => {
                        var totalValue = 0;
                        let txt = "<ul>";
                        querySnapshot.forEach(doc => {
                            ...
                            fetch(url).then(res => {
                                ...
                                if (data.latestPrice > data.previousClose) {
                                    txt += "<font color = 'green'>";

                                } else if (data.latestPrice < data.previousClose) {
                                    txt += "<li><font color = 'red'>";

                                }else{
                                    txt += "<li><font color = 'gray'>";
                                }
                                ...
                                var value = doc.data().currPrice * doc.data().qty;
                                totalValue = totalValue + value;
                                txt += doc.id + " " + doc.data().qty + " shares $" + value + "</font></li>";
                                txt += "</ul>"
                                return [txt, totalValue];
                            }).then(res => {
                                let txt = "<h1>Portfolio (" + res[1] + ")</h1>";
                                var stocklist = document.getElementById("stockList");
                                stocklist.innerHTML = txt + res[0];
